...
这个脚本只在标定导出时主动打开相机 SDK。生产运行不应调用它；
hand_eye_3D 的默认 ZMQ 路径只读取本脚本生成的 JSON。
"""
...
def export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    color_profile = _select_profile(
        ...
        fps=args.color_fps if args.color_fps is not None else args.fps,
    ...
    depth_profile = _select_profile(
        ...
        fps=args.depth_fps if args.depth_fps is not None else args.fps,
    ...
    payload = {
        ...
        "generator": "hand_eye_3D/tools/export_orbbec_rgbd_calibration.py",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="彩色流 FPS；未指定时使用 --fps",
    )
    parser.add_argument(
        "--depth-fps",
        type=int,
        default=None,
        help="深度流 FPS；未指定时使用 --fps",
    )
